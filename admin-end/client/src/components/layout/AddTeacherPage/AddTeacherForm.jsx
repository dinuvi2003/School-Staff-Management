...
import { User, Mail, Phone, MapPin, Calendar, Users, GraduationCap, Upload, X, CreditCard } from "lucide-react";
...
export default function  AddTeacherForm() {
  ...
  const onImageChange = (e) => {
    ...
    const MAX = 2 * 1024 * 1024; // 2MB (tweak as you like)
    if (file.size > MAX) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // --- Option 1: JSON submit (no file upload, just preview). Simple & works now.
      const payload = {
      ...
      // --- Option 2 (future): multipart upload to actually send the image file
      // const fd = new FormData();
      // Object.entries(payload).forEach(([k, v]) => fd.append(k, v ?? ""));
      // if (profileFile) fd.append("profileImage", profileFile);
      // const res = await addTeacher(fd, true);
  ...
  const InputWithIcon = ({ label, name, type = "text", required, placeholder, icon: Icon }) => (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input
          ...
          ${errors[name] ? "border-red-300 bg-red-50 focus:border-red-500" : "border-gray-300 hover:border-gray-400 focus:border-indigo-500"}`}
        />
      </div>
      {errors[name] && (
  ...
  const SelectWithIcon = ({ label, name, required, children, icon: Icon, disabled = false }) => (
    <div className="space-y-1.5">
      <label htmlFor={name} className="block text-sm font-medium text-gray-700">
        {label}{required && <span className="text-red-500 ml-1">*</span>}
      </label>
      <div className="relative">
        ...
        <select
          ...
          ${errors[name] ? "border-red-300 bg-red-50 focus:border-red-500"
            ...
            <div className="flex items-center">
              <div
                className={`flex-shrink-0 w-5 h-5 rounded-full ${
                  serverMsg.type === "success" ? "bg-green-100" : "bg-red-100"
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100">
          <form onSubmit={onSubmit} noValidate className="p-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="lg:col-span-2">
                <InputWithIcon label="Full Name" name="fullName" required placeholder="K.D.M. Paranagama"/>
              </div>

              <div className="lg:col-span-2">
                <InputWithIcon label="National Identity Card Number" name="nic" required placeholder="1997XXXXXXXX"/>
              </div>

              <InputWithIcon label="Date of Birth" name="dob" type="date" />

              <SelectWithIcon label="Gender" name="gender" required>
              ...
              <InputWithIcon label="Contact Number" name="phone" required placeholder="07XXXXXXXX" />
              <InputWithIcon label="Email Address" name="email" type="email" required placeholder="sample@gmail.com" />

              <div className="lg:col-span-2">
                <InputWithIcon label="Address" name="address" required placeholder="177/1 B, Vihara Mawatha, Malabe" />
              </div>
              ...
              <SelectWithIcon label="Service" name="service" required>
              ...
              <div className="lg:col-span-2">
                <InputWithIcon label="First Appointment Date" name="joinDate" type="date" required />
              </div>
            </div>
            ...
            <div className="mt-10 flex justify-center">
              <button
                type="submit"
                disabled={submitting}
                className={`inline-flex items-center gap-3 px-8 py-3 rounded-xl font-semibold text-base transition-all duration-200 shadow-lg ${
                  submitting
