...
export async function createInviteToken(user_id, token_hash, ttlHours) {
    ...
    const { data, error } = await db.from('action_token')
...
export async function consumeInviteToken(token_hash) {
    const { data, error } = await db
        .from('action_token') // ensure this matches your insert table name
        .update({ used_at: new Date().toISOString() })
        .eq('purpose', 'invite')
        .eq('token_hash', token_hash)
        .is('used_at', null)
    ...
    if (!data) {
        return { error: { message: 'Invalid or expired invite token' }, data: null };
    }
...
export async function validateInviteToken(token_hash) {
    const { data, error } = await db
        .from('action_token')
        .select('*')
        .eq('purpose', 'invite')
    ...
    // Note: maybeSingle() returns null data if no match, instead of error
    if (error) {
        return { error, data: null };
    }
    return { data, error: null };
