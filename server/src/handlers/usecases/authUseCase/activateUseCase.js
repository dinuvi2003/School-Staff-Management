import { hashPassword, hashToken } from "../../../services/utils/crypto.js";
...
export async function activateUseCase({ token, user_id, password }) {
    ...
    console.log("Hashed token:", token_hash);
    const { data: consumedToken, error: tokenError } = await consumeInviteToken(token_hash);

    console.log("Consumed invite token:", consumedToken);
    ...
    if (String(consumedToken.user_id) !== String(user_id)) {
        return {
            ok: false,
            status: 400,
